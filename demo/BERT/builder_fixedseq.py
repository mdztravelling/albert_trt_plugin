...
emln_plg_creator = plg_registry.get_plugin_creator("CustomEmbFactorizedLayerNormPluginDynamic", "1", "")
...
def attention_layer_opt(prefix, config, init_dict, network, input_tensor, imask, layer_index):
    """
    ...
    set_output_name(mult_all, prefix, "l{}_qkv_mult".format(layer_index))
    ...
    set_output_name(qkv2ctx, prefix, "l{}_context_layer".format(layer_index))
    return qkv2ctx
...
def transformer_layer_opt(prefix, config, init_dict, network, input_tensor, imask, layer_index):
    """
    ...
    context_transposed = attention_layer_opt(prefix + "attention_", config, init_dict, network, input_tensor, imask, layer_index)
    ...
    set_tensor_name(intermediate_act, prefix, "_l{}_gelu".format(layer_index))
    ...
    #set_tensor_name(set_tensor_name, prefix, "_l{}_relu".format(layer_index))
    ...
    set_output_name(out_dense, prefix + "_l{}_output_".format(layer_index), "dense")

    out_layer = skipln(prefix + "output_layernorm_", config, init_dict, network, out_dense.get_output(0), attention_ln, B_lout)
    set_output_name(out_layer, prefix + "_l{}_output_".format(layer_index), "reshape")
...
def bert_model(config, init_dict, network, input_tensor, input_mask):
    ...
    for layer in range(0, config.num_hidden_layers):
        ss = "layer_shared_"
        out_layer = transformer_layer_opt(ss, config,  init_dict, network, prev_input, input_mask, layer)
...
def emb_layernorm(builder, network, config, weights_dict, builder_config, sequence_lengths, batch_sizes):
    ...
    wwordemb2 = trt.PluginField("bert_embeddings_word_embeddings_2", weights_dict["bert_embeddings_word_embeddings_2"].numpy(), trt.PluginFieldType.FLOAT32)
    ...
    pfc = trt.PluginFieldCollection([wbeta, wgamma, wwordemb, wtokemb, wposemb, wwordemb2, output_fp16, mha_type])
    fn = emln_plg_creator.create_plugin("factorized_embeddings", pfc)
...
def build_engine(batch_sizes, workspace_size, sequence_lengths, config, weights_dict, squad_json, vocab_file, calibrationCacheFile, calib_num):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
        ...
        squad_logits = squad_output("loss/cls_", config, weights_dict, network, bert_out)
