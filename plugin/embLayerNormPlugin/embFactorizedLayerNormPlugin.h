...
#ifndef TRT_EMB_FACTORIZED_LAYER_NORM_PLUGIN_H
#define TRT_EMB_FACTORIZED_LAYER_NORM_PLUGIN_H
...
template<typename T>
int wordEmbLayer(cudaStream_t stream, int ld, int B, int S, const int* inputIds, const T* wordEmb, T* output);
...
template<typename T>
int tuneBatchedGemmEx(const int B, const int S, const int embedding_size, const int headSize);

class EmbFactorizedLayerNormPluginDynamic : public nvinfer1::IPluginV2DynamicExt
{
public:
    EmbFactorizedLayerNormPluginDynamic(const std::string& name, const nvinfer1::DataType type, const nvinfer1::DataType mhaType,
        const nvinfer1::Weights& beta, const nvinfer1::Weights& gamma, const nvinfer1::Weights& word_emb,
        const nvinfer1::Weights& pos_emb, const nvinfer1::Weights& tok_emb, const nvinfer1::Weights& wordEmb2, const bool useFullMask);

    EmbFactorizedLayerNormPluginDynamic(const std::string& name, const void* data, size_t length);

    // It doesn't make sense to make EmbFactorizedLayerNormPluginDynamic without arguments, so we
    // delete default constructor.
    EmbFactorizedLayerNormPluginDynamic() = delete;
...
class EmbFactorizedLayerNormPluginDynamicCreator : public nvinfer1::IPluginCreator
{
public:
    EmbFactorizedLayerNormPluginDynamicCreator();
...
#endif // TRT_EMB_FACTORIZED_LAYER_NORM_PLUGIN_H
