    ...
    const float* gamma, const T* wordEmb, const T* posEmb, const T* tokEmb, T* output)
{
    ...
    //const int seqPos = blockIdx.x + blockIdx.y * gridDim.x;  // mdz
    if (threadIdx.x == 0)
    {
...
template <typename T, unsigned TPB>
__global__ void embLayerKernel(int emb_sz, const int* inputIds, const T* wordEmb, T* output)
{
    ...
    // 2. load pos/tok/word embeddings and add them toghether
    // offset into embeddings is given by wordId * embedding_size
    const int woffset = wordId * emb_sz;
    // the output offset is given by b * (S*embedding_size) + s * embedding_size
    const int outOffset = seqPos * emb_sz;

    for (int it = threadIdx.x; it < emb_sz; it += TPB)
    {
      output[outOffset + it] = wordEmb[woffset + it];
    }
}

template <typename T, unsigned TPB>
__global__ void embLayerNormKernelV2(int ld, const int* tokenIds, const float* beta,
    const float* gamma, const T* wordEmb, const T* posEmb, const T* tokEmb, T* output)
{
    ...
    for (int it = threadIdx.x; it < ld; it += TPB)
    {
        const T w(wordEmb[outOffset + it]);
        const T t(tokEmb[toffset + it]);
        const T p(posEmb[poffset + it]);
        const T val = w + t + p;
...
template <typename T>
int wordEmbLayer(cudaStream_t stream, int emb_sz, int B, int S, const int* inputIds, const T* wordEmb, T* output) {
    constexpr int tpb = 64;
    const dim3 grid(S, B, 1);
    const dim3 block(tpb, 1, 1);

    embLayerKernel<T, tpb><<<grid, block, 0, stream>>>(emb_sz, inputIds, wordEmb, output);
    CHECK(cudaPeekAtLastError());
    return 0;
}

template int wordEmbLayer<float>(
    cudaStream_t stream, int emb_sz, int B, int S, const int* inputIds, const float* wordEmb, float* output);
template int wordEmbLayer<half>(
    cudaStream_t stream, int emb_sz, int B, int S, const int* inputIds, const half* wordEmb, half* output);

template <typename T>
int embSkipLayerNormV2(cudaStream_t stream, int ld, int B, int S, const int* token_ids,
    const float* beta, const float* gamma, const T* wordEmb, const T* posEmb, const T* tokEmb, T* output)
{
    constexpr int tpb = 256;
    const dim3 grid(S, B, 1);
    ...
    embLayerNormKernelV2<T, tpb>
        <<<grid, block, 0, stream>>>(ld, token_ids, beta, gamma, wordEmb, posEmb, tokEmb, output);
    CHECK(cudaPeekAtLastError());

    return 0;
}

template int embSkipLayerNormV2<float>(cudaStream_t, int, int, int, const int*, const float*, const float*,
    const float*, const float*, const float*, float*);

template int embSkipLayerNormV2<half>(cudaStream_t, int, int, int, const int*, const float*, const float*,
    const half*, const half*, const half*, half*);
